...
const JoinTeamModal = ({ isVisible, onClose, onJoinTeam, showNotification }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/projects/join', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        onJoinTeam(teamCode);
        showNotification('success', data.message || `Successfully joined team with code: ${teamCode}`);
      } else {
        showNotification('error', data.message || 'Invalid team code. Please check and try again.');
      }
    } catch (error) {
